          ...
          ins =>
            if(ins.isInstanceOf[PTAConcreteStringInstance]){
              if(CryptographicConstants.getECBSchemes.contains(ins.asInstanceOf[PTAConcreteStringInstance].string))
                result += (node -> false)//Need to change this ##########
            }
        }
    }
    result
                ...
                if (callee.signature.contains(".setComponentEnabledSetting")) {
                  ...
                  if (valueArrayForParam2(0) == 2) {
                    ...
                    if (valueArrayForParam3(0) == 1) {
                      ...
                      scala.tools.nsc.io.File("C:/Users/Netorare/Desktop/GAWork/Amandroid/HideAPI/results.txt").appendAll(fileName(fileName.length-1) +" has Hide Icon API! \n")
