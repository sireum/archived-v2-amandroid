...
import org.sireum.jawa.alir.interProcedural.objectFlowAnalysis._
...
import org.sireum.jawa.alir.intraProcedural.compressedControlFlowGraph.CompressedControlFlowGraph
...
//        sCfg.collectionCCfgToBaseGraph(callee, calleeCCfg)
    ...
    if(processed.contains(calleeSig, callerContext)){
      ...
      if(!ofg.isModelOperation(calleeSig))
      	sCfg.extendGraph(calleeSig, callerContext.copy)
    } else {
      //need to extend
    }
  }
