              ...
              intentData =>
                intentData.getType match{
                  case intentData.EXPLICIT => 
                    ...
                    if(!intentData.targets.isEmpty && intentData.targets.forall(_._2 == intentData.EXPLICIT)) explicitIccTargetFound += 1
                  case intentData.IMPLICIT =>
                    implicitIccTotal += 1
                    if(intentData.preciseImplicit) implicitIccPrecise += 1
                    if(!intentData.targets.isEmpty && intentData.targets.forall(_._2 == intentData.IMPLICIT)) implicitIccTargetFound += 1
                  case intentData.MIXED =>
                    mixedIccTotal += 1
                    if(intentData.preciseExplicit && intentData.preciseImplicit) mixedIccPrecise += 1
                    if(!intentData.targets.isEmpty 
