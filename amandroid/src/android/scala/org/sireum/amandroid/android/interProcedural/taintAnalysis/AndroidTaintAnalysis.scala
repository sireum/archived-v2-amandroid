...
import org.sireum.amandroid.interProcedural.reachingFactsAnalysis.HeapSlot
import org.sireum.amandroid.MessageCenter._
import org.sireum.amandroid.interProcedural.reachingFactsAnalysis.FieldSlot
import org.sireum.amandroid.interProcedural.reachingFactsAnalysis.ArraySlot
...
//    print(result)
  ...
  private def getCalleeSet(s : ISet[TaintFact], cj : CallJump, callerContext : Context) : ISet[AmandroidProcedure] = {
  ...
  private def getFactsForICCTarget(s : ISet[TaintFact], cj : CallJump, callee : AmandroidProcedure, callerContext : Context) : ISet[TaintFact] = {
  ...
  private def getFactsForCallee(s : ISet[TaintFact], cj : CallJump, callee : AmandroidProcedure, callerContext : Context) : ISet[TaintFact] = {
    var calleeFacts = isetEmpty[TaintFact]
    val rfacts = this.rfaFacts.entrySet(this.cg.getCGCallNode(callerContext))
    val globalFacts = ReachingFactsAnalysisHelper.getGlobalFacts(rfacts)
    calleeFacts ++= s.filter(taFact => globalFacts.contains(taFact.fact))
