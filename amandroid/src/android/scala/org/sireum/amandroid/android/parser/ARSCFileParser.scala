...
object ARSCFileParser extends AbstractAndroidXMLParser {
	...
	def parse(apkFile : String) = {
		this.handleAndroidXMLFiles(apkFile, Set("resources.arsc"), new AndroidXMLHandler() {
	...
	private def readResourceHeader(stream : InputStream) : Unit = {
		...
		var remainingData = new Array[Byte](remainingSize)
							...
							if (entryOffset == 0xFFFF){}	// NoEntry
							  ...
							  var flag = true
								...
								if (entry.flagsComplex) {
									...
									for (j <- 0 to entry.asInstanceOf[ResTable_Map_Entry].count - 1) {
	...
	private def readComplexValue(map : ResTable_Map, remainingData : Array[Byte], offset : Int) : Int = {
	  var temp = offset
		...
		return readValue(map.value, remainingData, temp)
	}
	...
	private def readEntryTable(data : Array[Byte], offset : Int) : ResTable_Entry = {
	  ...
	  var temp = offset
		...
		if (entry.isInstanceOf[ResTable_Map_Entry]) {
			val mapEntry = entry.asInstanceOf[ResTable_Map_Entry]
	...
	private def readStringTable(remainingData : Array[Byte], offset : Int, blockStart : Int, 
	  ...
	  var temp = offset
		// Read the strings
		for (i <- 0 to stringPoolHeader.stringCount - 1) {
