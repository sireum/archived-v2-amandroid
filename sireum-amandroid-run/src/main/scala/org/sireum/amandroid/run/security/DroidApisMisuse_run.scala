...
import org.sireum.amandroid.security.apiMisuse.DroidApisMisuse
 ...
 */
object DroidApisMisuse_run {
  private final val TITLE = "DroidApisMisuse_run"
  
  object DroidApisMisuseCounter {
    var total = 0
    var oversize = 0
    var haveresult = 0
  ...
  private class DroidApisMisuseListener extends AmandroidSocketListener {
    def onPreAnalysis: Unit = {
      DroidApisMisuseCounter.total += 1
    }
    ...
    def onAnalysisSuccess : Unit = {
		  DroidApisMisuseCounter.haveresult += 1
    }

    def onPostAnalysis: Unit = {
      msg_critical(TITLE, DroidApisMisuseCounter.toString)
    }
    ...
    files.foreach{
      file =>
        try{
          ...
          socket.plugListener(new DroidApisMisuseListener)
          ...
          icfgs.foreach{
            case (rec, InterProceduralDataFlowGraph(icfg, irfaResult)) =>
              DroidApisMisuse(new InterProceduralDataFlowGraph(icfg, irfaResult))
          }
        } catch {
          case e : Throwable =>
            e.printStackTrace()
